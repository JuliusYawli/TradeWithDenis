      ...
      <Link href={`/iphones/${product.slug}`} className="block">
        <div className="relative aspect-[4/3] bg-gradient-to-br from-snow to-white">
          ...
          <div className="absolute left-3 top-3 flex flex-wrap gap-2">
            <span className="rounded-full bg-white/90 px-2.5 py-1 text-xs font-semibold capitalize text-red shadow-sm backdrop-blur">{product.stock_status.replace("_", " ")}</span>
            <span className="rounded-full bg-ink/85 px-2.5 py-1 text-xs font-semibold text-white shadow-sm backdrop-blur">{product.condition}</span>
          </div>
        </div>
      </Link>
      <div className="space-y-4 p-4 sm:p-5">
        ...
        <div className="rounded-lg border border-line bg-snow p-3 text-sm">
          <div>
            ...
            <p className="text-xl font-semibold text-ink">{formatCedi(product.price)}</p>
          </div>
          <div className="mt-3 grid grid-cols-3 gap-2 border-t border-line pt-3">
            <div>
              <p className="text-neutral-500">Weekly</p>
              <p className="font-medium">{formatCedi(product.weekly_payment)}</p>
            </div>
            <div>
              <p className="text-neutral-500">Deposit</p>
              <p className="font-medium">{formatCedi(finance.downPayment)}</p>
            </div>
            <div>
              <p className="text-neutral-500">Term</p>
              <p className="font-medium">{product.installment_weeks} wks</p>
            </div>
          </div>
        </div>
        ...
        <p className="inline-flex items-center gap-2 text-xs font-medium text-neutral-500"><ShieldCheck className="h-3.5 w-3.5 text-gold" /> Complete purchase in shop after inspection.</p>
      </div>
    </article>
  );
}
